...
const HeroSection = () => {
  ...
  return <section className="relative h-[60vh] sm:h-[70vh] md:h-[80vh] w-full overflow-hidden">
      ...
      <div className="absolute inset-0 flex items-center justify-between z-30 px-2 sm:px-6">
        <button onClick={goToPrevSlide} className="bg-black/30 hover:bg-black/50 text-white rounded-full p-1 sm:p-2 transition-colors" aria-label="Previous image">
          <ChevronLeft className="h-5 w-5 sm:h-6 sm:w-6" />
        </button>
        <button onClick={goToNextSlide} className="bg-black/30 hover:bg-black/50 text-white rounded-full p-1 sm:p-2 transition-colors" aria-label="Next image">
